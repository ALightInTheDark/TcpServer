// trivial
// Created by kiki on 2021/11/18.18:16
#pragma once
...
private:
	Reactor& main_reactor; // 主线程中创建的Reactor对象, 处理Acceptor的IO事件，即accept。main_reactor必须由主线程创建，不能从IO线程池获取，否则主线程将不执行任何任务。
private:
	vector<thread> thread_vector;
	vector<Reactor*> sub_reactor_vector; // 处理TcpConnection的IO事件。对于IO密集型的服务，轻量的计算任务可由IO线程代为处理。而计算密集型的服务，应交由计算线程池处理。
