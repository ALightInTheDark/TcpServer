  ...
  self_addr(self), peer_addr(peer)
{
	...
	int opt =  1;
	int ret = ::setsockopt(conn_fd.fd, SOL_SOCKET, SO_KEEPALIVE, &opt, static_cast<socklen_t>(sizeof opt)); //KeepAlive定期探测连接是否存在，如果应用层有心跳，这个选项就不必设置
...
void TcpConnection::HandleRead()
{
	...
	else { message_callback(shared_from_this()); } // 用户的message_callback可以把任务提交给计算线程池，计算完毕后（计算线程）调用TCPConnection::SendAcrossThreads(), 把结果发送给客户端。
}
void TcpConnection::HandleWrite()
{if (!reactor.InIOThread()) { fatal("不能够跨线程调用HandleWrite函数！") } if (!event_register.MonitoringWritable()) { fatal("call HandleWrite() but not MonitoringWritable") }
	ssize_t n = write(conn_fd.fd, write_buffer.readable_ptr(), write_buffer.ReadableBytes()); // 这里确实是readable_ptr()，而不是writable_ptr
	...
	if (write_buffer.ReadableBytes() == 0) // 应用层发送缓冲区已清空
	{
		...
		if (state == ConnectionState::Disconnecting) { shutdown(conn_fd.fd, SHUT_WR); } // fixme:改写了muduo
	}
}
...
void TcpConnection::HandleError() const
{
	...
	int ret = getsockopt(conn_fd.fd, SOL_SOCKET, SO_ERROR, &opt, &len);
...
{if(!reactor.InIOThread()) { fatal("必须在IO线程中调用！") }
...
{if(!reactor.InIOThread()) { fatal("必须在IO线程中调用！") } if (state != ConnectionState::Connecting) { fatal("state != ConnectionState::Connecting！")}
	state = ConnectionState::Connected;

	event_register.hold(shared_from_this());
	event_register.InterestReadableEvent();
...
{if (!reactor.InIOThread()) { fatal("不能够跨线程调用Send函数！") } if (state == ConnectionState::Disconnected) { err("call send at Disconnected TcpConnection") return; }
	...
	if (write_buffer.ReadableBytes() == 0 && !event_register.MonitoringWritable()) // 内核发送缓冲区尚有空间，直接发送。
	{
		...
		if (wrote_bytes != -1)
		{
			...
			{ reactor.AddTask([this]{write_complete_callback(shared_from_this());} ); } //在write_complete_callback调用Send函数，向客户端发送数据（见CharacterGenerator.cpp），如果是reactor.Execute（...）而不是reactor.AddTask(...)，则会循环调用Send()和write_complete_callback，进而爆栈。
		}
